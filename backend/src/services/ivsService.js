...
class IVSService {
  ...
  async listChannels(region) {
    try {
      ...
      return { success: true, data: { channels: res.channels.map(ch => ({ arn: ch.arn, id: ch.channelId, name: ch.name, region, state: ch.state, ingestEndpoint: ch.ingestEndpoint, playbackUrl: ch.playbackUrl, streamKey: null, createdAt: ch.createdAt })) } };
    } catch (e) { return { success: false, error: e.message }; }
  }
